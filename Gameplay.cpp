...
//checkUMatching(IsExist, x0, y0, x1, y1, ROWS, COLS)
...
bool CheckZMatching(bool IsExist[][10], int x0, int y0, int x1, int y1) {
	...
	for (int i = x0 + 1;i < x1;++i) {
		if (IsExist[i][y0] == false)break;
		if (y0 > y1) {
			for (int j = y0 - 1;j > y1;j--) if (IsExist[i][j] == false) continue;
		}
		else{
			for (int j = y0 + 1;j < y1;j++) if (IsExist[i][j] == false) continue;
		}
		for (int j = i + 1; j < x1;++j) {
			if (IsExist[j][y1] == false) continue;
		}
		return true;
	}
	if (y0 > y1)
	...
	for (int i = y0 + 1;i < y1;++i) {
		if (IsExist[x0][i] == false)break;
		if (x0 > x1) {
			for (int j = x0 - 1;j > x1;j--) if (IsExist[j][i] == false) continue;
		}
		else {
			for (int j = x0 + 1;j < x1;j++) if (IsExist[j][i] == false) continue;
		}
		for (int j = i + 1; j < y1;++j) {
			if (IsExist[x1][j] == false) continue;
		}
		return true;
	}
	return false;
